	...
	if ( is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) && is_plugin_active( 'mailjet-for-wordpress/wp-mailjet.php') ) {
		...
		<div class="wrap ydumailjet">
			...
			<form action="" method="POST" class="parameter">
				<h2><?php echo __('Réglages principaux', 'ydumailjet'); ?></h2>
				<?php
				...
				?>
				<p><?php _e("Vous avez besoin de renseigner quelques informations pour que le lien entre Mailjet et Contact Form 7 se fasse. Tout d'abord, il faut récupérer au niveau de Contact Form 7 le nom des champs pour l'email, le nom, et optionnellement la case à cocher qui confirme que le visiteur s'inscrit bien à la newsletter.", 'ydumailjet'); ?></p>
				<p><?php _e('Ensuite, vous devez renseigner le numéro de la liste de contact à laquelle vous souhaitez connecter Contact Form 7. Vous trouverez le lien ci-après pour cela :', 'ydumailjet'); ?></p>
				<p><a href="https://app.mailjet.com/contacts" target="_blank"><?php _e('lien vers la page contacts de Mailjet', 'ydumailjet'); ?></a></p>
				<p><?php _e('Il est possible de faire des réglages différents en fonction de vos formulaires, ainsi vous pouvez gérer des cas de figure différents comme une liste de contacts Mailjet différente ou des noms de champs différents. La liste déroulante "Formulaires" permet de choisir les formulaires auxquels sont associés les réglages que vous venez de faire.', 'ydumailjet'); ?></p>
				...
				<?php if (!class_exists('WP_Mailjet_Api_Strategy_V3')) { ?>
					<h2><?php echo __('Réglages secondaires', 'ydumailjet'); ?></h2>
					<p><?php _e("Il est possible avec Mailjet de connecter plus de données que seulement le nom et l'email. Cette partie est conçue pour cela. Indiquez, dans un premier temps, le nom que vous souhaitez donner à la donnée dans Mailjet, puis, dans un second temps, le nom du champ dans Contact Form 7.", 'ydumailjet'); ?></p>
					<div class="ydusecondary"><label><?php echo __('1 - Label', 'ydumailjet'); ?></label> <input type="" name="custom_1_label" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_1_label')); ?>" />
					<label><?php echo __('1 - Contact form 7 field', 'ydumailjet'); ?></label> <input type="" name="custom_1_field" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_1_field')); ?>" /><br/></div>
					<div class="ydusecondary"><label><?php echo __('2 - Label', 'ydumailjet'); ?></label> <input type="" name="custom_2_label" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_2_label')); ?>" />
					<label><?php echo __('2 - Contact form 7 field', 'ydumailjet'); ?></label> <input type="" name="custom_2_field" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_2_field')); ?>" /><br/></div>
					<div class="ydusecondary"><label><?php echo __('3 - Label', 'ydumailjet'); ?></label> <input type="" name="custom_3_label" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_3_label')); ?>" />
					<label><?php echo __('3 - Contact form 7 field', 'ydumailjet'); ?></label> <input type="" name="custom_3_field" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_3_field')); ?>" /><br/></div>
					<div class="ydusecondary"><label><?php echo __('4 - Label', 'ydumailjet'); ?></label> <input type="" name="custom_4_label" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_4_label')); ?>" />
					<label><?php echo __('4 - Contact form 7 field', 'ydumailjet'); ?></label> <input type="" name="custom_4_field" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_4_field')); ?>" /><br/></div>
					<div class="ydusecondary"><label><?php echo __('5 - Label', 'ydumailjet'); ?></label> <input type="" name="custom_5_label" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_5_label')); ?>" />
					<label><?php echo __('5 - Contact form 7 field', 'ydumailjet'); ?></label> <input type="" name="custom_5_field" value="<?php echo esc_attr(get_option('mailjet_form_7_custom_5_field')); ?>" /></div>
				<?php } ?>
				<div class="clearfix"></div>
...
function mailjet_add_to_list($list, $contacts)
{
	...
	} else if (class_exists('MailjetWp\MailjetPlugin\Includes\MailjetApi')) {
	...
	} else if (class_exists('MailjetPlugin\Includes\MailjetApi')) {
		//Wordpress 5.0
